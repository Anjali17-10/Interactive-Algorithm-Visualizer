...
if(user){
  ...
  fetch(`https://interactive-algorithm-visualizer-9.onrender.com/api/progress/${user.email}`)
    .then(res => res.json())
    .then(progress => {
...
const response = await fetch(
  `https://interactive-algorithm-visualizer-9.onrender.com/api/progress/${user.email}`
);
...
async function markCompleted(algorithm){
    ...
    try{
      ...
      const response = await fetch(
  "https://interactive-algorithm-visualizer-9.onrender.com/api/progress/complete",
  {
                method: "POST",
                headers: {
